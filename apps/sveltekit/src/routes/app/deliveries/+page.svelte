<script lang="ts">
  ...
  const deliveries = $derived.by(() => {
    ...
    if (activeTab === 'in_transit')
      return items.filter((d: any) => d.status === 'in_transit' || d.status === 'on_site');
    if (activeTab === 'received')
      return items.filter((d: any) => d.status === 'received_inspected');
    return items;
  });
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        Monitor and manage material deliveries in real time
      </p>
    </div>
  ...
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
    ...
    <Card padding="lg">
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        Confirm material arrival with photo and signature
      </p>
      <div
        class="mt-4 flex h-48 items-center justify-center rounded-md border border-dashed border-border bg-muted/30"
      >
...
<Modal bind:open={showCreateModal} title="New Delivery">
  <div class="space-y-4">
    ...
    <Input
      ...
      bind:value={newDelivery.materials}
      placeholder="e.g. 10×Steel beams, 5×Concrete blocks"
    />
    <Input
      label="ETA (timestamp ms)"
      bind:value={newDelivery.eta}
      placeholder="e.g. 1717000000000"
    />
    <Button
      onclick={async () => {
