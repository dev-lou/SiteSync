  ...
  try {
    ...
    if (viewerContainer.value) {
      viewer = OpenSeadragon.default({
        ...
        tileSources: {
          type: 'image',
          url: activeRevision.value?.fileUrl || 'https://via.placeholder.com/1600x1200.png?text=Blueprint',
        },
        showNavigator: true,
...
<template>
  <div class="blueprint-viewer relative" :class="{ 'fixed inset-0 z-50 bg-black': isFullscreen }">
    <!-- Loading state -->
    <div v-if="loading" class="flex h-96 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
    ...
    <!-- Error state -->
    <div v-else-if="error" class="flex h-96 flex-col items-center justify-center gap-3 rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
      <p class="text-sm text-red-500">{{ error }}</p>
      <button @click="initViewer()"
    ...
    <template v-else>
      ...
      <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2">
          ...
          <span v-if="safeBlueprint" class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
            :class="{
              'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400': safeBlueprint.status === 'draft',
              'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400': safeBlueprint.status === 'in_review',
              'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': safeBlueprint.status === 'approved',
              'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400': safeBlueprint.status === 'for_construction',
            }">
        ...
        <!-- Zoom controls -->
        <div class="flex items-center gap-1 rounded-lg border border-gray-200 bg-white p-0.5 dark:border-gray-700 dark:bg-gray-900">
          <button @click="zoomOut" title="Zoom out"
            class="flex h-7 w-7 items-center justify-center rounded text-xs hover:bg-gray-100 dark:hover:bg-gray-800">−</button>
          <span class="min-w-[3rem] text-center text-xs font-medium text-gray-600 dark:text-gray-400">{{ zoomLevel }}x</span>
      ...
      <div class="mb-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <span>Rev {{ activeRevision?.revisionNumber || safeBlueprint?.currentRevision || '—' }}</span>
        <span v-if="activeRevision?.uploadedAt">&middot; {{ formatDate(activeRevision.uploadedAt) }}</span>
        <span v-if="activeRevision?.changeLog" class="italic">&middot; "{{ activeRevision.changeLog }}"</span>

        <!-- Revision switcher -->
        <button @click="showRevisions = !showRevisions"
          ...
          <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg>
      ...
      <div class="relative" :class="{ 'h-dvh': isFullscreen }">
        ...
        <div v-if="!loading && !viewer" class="flex h-32 items-center justify-center">
          <p class="text-sm text-muted-foreground">Select a revision to view</p>
        </div>
      </div>

      <!-- Revision dropdown -->
      <div v-if="showRevisions" class="absolute left-0 right-0 top-full z-20 mt-1 rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-900">
        <div class="max-h-48 overflow-y-auto p-2">
          <button v-for="rev in safeRevisions" :key="rev._id"
            ...
            :class="{ 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400': rev.revisionNumber === activeRevision?.revisionNumber }">
            <div>
              <p class="font-medium">Revision {{ rev.revisionNumber }}</p>
              <p v-if="rev.changeLog" class="text-xs text-muted-foreground">{{ rev.changeLog }}</p>
            </div>
            <span class="text-xs text-muted-foreground">{{ formatDate(rev.uploadedAt) }}</span>
          </button>
          <p v-if="safeRevisions.length === 0" class="p-3 text-center text-sm text-muted-foreground">No revisions uploaded yet.</p>
        </div>
      </div>
    </template>
  </div>
