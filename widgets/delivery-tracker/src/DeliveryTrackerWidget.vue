...
const filteredDeliveries = computed(() => {
  ...
  switch (activeTab.value) {
    case 'ordered':
      return all.filter((d) => d.status === 'ordered' || d.status === 'dispatched');
    case 'in_transit':
      return all.filter((d) => d.status === 'in_transit');
    case 'on_site':
      return all.filter((d) => d.status === 'on_site');
    case 'received':
      return all.filter((d) => d.status === 'received_inspected');
    default:
      return all;
  }
});
...
const onSiteDeliveries = computed(() => deliveries.value.filter((d) => d.status === 'on_site'));
...
function updateMarkers() {
  ...
  for (const d of inTransitDeliveries.value) {
    ...
    if (pos) {
      ...
      const marker = L.marker([pos.lat, pos.lng], { icon: rotatedIcon }).bindPopup(
        `<b>${d.title}</b><br/>${d.supplier}<br/>ETA: ${formatDate(d.eta)}`,
      );
...
<template>
  <div class="delivery-tracker">
    <!-- Tabs -->
    <div class="mb-4 flex gap-2 overflow-x-auto border-b border-gray-200 dark:border-gray-700">
      <button
        v-for="tab in [
          { id: 'all', label: 'All', count: deliveries.length },
          {
            id: 'ordered',
            label: 'Ordered',
            count: deliveries.filter((d) => d.status === 'ordered' || d.status === 'dispatched')
              .length,
          },
          {
            id: 'in_transit',
          ...
          },
          {
            id: 'on_site',
            label: 'On Site',
            count: deliveries.filter((d) => d.status === 'on_site').length,
          },
          {
            id: 'received',
            label: 'Received',
            count: deliveries.filter((d) => d.status === 'received_inspected').length,
          },
        ] as const"
        :key="tab.id"
      ...
      >
        ...
        <span
          ...
          class="absolute right-0 bottom-0 left-0 h-0.5 bg-blue-600"
        />
      </button>
    </div>
    ...
    >
      ...
      <p class="mt-1 text-xs">No deliveries match the current filter.</p>
    </div>
      ...
      <div class="mb-2 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300"
            >Live Tracking Map</span
          >
        </div>
        <span class="text-xs text-gray-400"
          >{{ inTransitDeliveries.length }} vehicle(s) in transit</span
        >
      </div>
      <div
        ref="mapContainer"
        class="z-0 h-64 w-full rounded-lg border border-gray-200 dark:border-gray-700"
      />
    </div>
    ...
    <Teleport to="body">
      <div
        v-if="showReceiptModal && selectedDelivery"
      ...
      >
        ...
        >
          ...
          >
            <div>
              ...
              <p class="text-sm text-gray-500">
                {{ selectedDelivery.title }} - {{ selectedDelivery.supplier }}
              </p>
            </div>
            <button
              @click="closeReceipt"
          ...
          <div class="space-y-5 p-5">
            ...
            <div>
              ...
              <div v-if="signatureDataUrl" class="mt-2">
                <img
                  ...
                  class="h-12 rounded border border-gray-200 dark:border-gray-700"
                />
              </div>
            </div>
            ...
            <p
              v-if="receiptError"
              class="rounded-md bg-red-50 px-3 py-2 text-sm text-red-500 dark:bg-red-900/20"
            >
              {{ receiptError }}
            </p>
            ...
            <div class="rounded-md bg-gray-50 p-3 dark:bg-gray-800">
              ...
              <div class="mt-1 space-y-1 text-sm">
                ...
                <p>
                  <span class="text-gray-500">ETA:</span> {{ formatDate(selectedDelivery.eta) }}
                </p>
                <p v-if="selectedDelivery.materialList?.length">
                  ...
                  <span v-for="(mat, idx) in selectedDelivery.materialList" :key="idx" class="ml-1">
                    ...
                    }}{{ idx < (selectedDelivery.materialList?.length || 0) - 1 ? ',' : '' }}
                  </span>
                </p>
              </div>
            </div>
          ...
          <div class="flex gap-3 border-t border-gray-200 px-5 py-4 dark:border-gray-700">
            ...
            <button
              ...
              class="flex flex-1 items-center justify-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700 disabled:opacity-50"
            >
