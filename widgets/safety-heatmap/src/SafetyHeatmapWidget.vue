...
const sampleZones: Zone[] = [
  { _id: 'zone-a', name: 'Zone A - Foundation', svgPath: 'M 50 50 L 200 50 L 200 180 L 50 180 Z', status: 'active' },
  { _id: 'zone-b', name: 'Zone B - Structural', svgPath: 'M 220 50 L 370 50 L 370 180 L 220 180 Z', status: 'suspended', suspendedReason: 'Structural integrity review' },
  { _id: 'zone-c', name: 'Zone C - Finishing', svgPath: 'M 50 200 L 200 200 L 200 320 L 50 320 Z', status: 'active' },
  { _id: 'zone-d', name: 'Zone D - MEP', svgPath: 'M 220 200 L 370 200 L 370 320 L 220 320 Z', status: 'completed' },
  { _id: 'zone-e', name: 'Zone E - Roof', svgPath: 'M 100 30 L 320 30 L 320 40 L 100 40 Z', status: 'active' },
];
...
<template>
  <div class="safety-heatmap relative">
    <!-- Loading state -->
    <div v-if="loading || zonesLoading" class="flex h-64 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
    ...
    <template v-else>
      ...
      <!-- SVG Floor Plan -->
      <div class="relative">
        <svg viewBox="0 0 420 360" class="w-full rounded-lg border border-gray-200 dark:border-gray-700" style="min-height: 320px">
          <!-- Background -->
          <rect x="0" y="0" width="420" height="360" rx="4" fill="#f9fafb" class="dark:fill-gray-800" />

          <!-- Grid lines for visual reference -->
          <line x1="0" y1="120" x2="420" y2="120" stroke="#e5e7eb" stroke-width="0.5" class="dark:stroke-gray-700" />
          <line x1="0" y1="240" x2="420" y2="240" stroke="#e5e7eb" stroke-width="0.5" class="dark:stroke-gray-700" />
          <line x1="210" y1="0" x2="210" y2="360" stroke="#e5e7eb" stroke-width="0.5" class="dark:stroke-gray-700" />

          <!-- Zone paths -->
          <g v-for="zone in displayZones" :key="zone._id"
            ...
            <g v-if="zone.status === 'suspended'">
              <rect :x="parseFloat(zone.svgPath.match(/M\s+(\d+)/)?.[1] || '0') + 5"
                :y="parseFloat(zone.svgPath.match(/L\s+\d+\s+(\d+)/)?.[1] || '0') + 5"
                width="16" height="14" rx="2" fill="#ef4444" opacity="0.9" />
              <path :d="`M${parseFloat(zone.svgPath.match(/M\s+(\d+)/)?.[1] || '0') + 8} ${parseFloat(zone.svgPath.match(/L\s+\d+\s+(\d+)/)?.[1] || '0') + 8} V${parseFloat(zone.svgPath.match(/L\s+\d+\s+(\d+)/)?.[1] || '0') + 3} a4 4 0 0 1 8 0 v5`"
            ...
            <text
              ...
              text-anchor="middle"
              class="fill-gray-700 text-[9px] font-medium dark:fill-gray-300 pointer-events-none"
          ...
          <!-- Tooltip on hover -->
          <g v-if="hoveredZone">
            <rect x="10" y="5" width="200" height="48" rx="4" fill="white" stroke="#e5e7eb" stroke-width="0.5" class="dark:fill-gray-900 dark:stroke-gray-700" />
            ...
            <text x="18" y="50" class="fill-gray-500 text-[9px] dark:fill-gray-400">
              {{ displayZones.find(z => z._id === hoveredZone)?.suspendedReason || 'Click to view permits' }}
            </text>
          </g>
        </svg>
      </div>
      ...
      <div v-if="showPanel && selectedZone"
        ...
        <div class="flex h-full flex-col">
          ...
          <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-700">
            <div>
              ...
              <span class="inline-flex items-center gap-1 text-xs"
                ...
                <span class="h-2 w-2 rounded-full" :style="{ background: zoneColors[selectedZone.status] }" />
                {{ selectedZone.status }}
              </span>
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-4">
            <div v-if="selectedZone.suspendedReason" class="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm dark:border-red-800 dark:bg-red-900/20">
            ...
            <h4 class="mb-2 text-sm font-semibold text-muted-foreground">Active Permits</h4>
            <div v-if="safeZonePermits.length > 0" class="space-y-2">
              <div v-for="permit in safeZonePermits" :key="permit._id"
                class="flex items-center justify-between rounded-md border border-gray-200 p-3 dark:border-gray-700"
                :class="{ 'border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-900/10': isExpiring(permit.expiresAt) }">
                <div>
                  <p class="text-sm font-medium">{{ permit.type.replace('_', ' ') }}</p>
                  <p class="text-xs text-muted-foreground">
                    Expires {{ formatDate(permit.expiresAt) }}
                    <span v-if="isExpiring(permit.expiresAt)" class="text-yellow-600 dark:text-yellow-400">(soon)</span>
                  </p>
                </div>
                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                  :class="{
                    'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': permit.status === 'active',
                    'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400': permit.status === 'applied',
                    'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400': permit.status === 'expired',
                  }">
                  {{ permit.status }}
                </span>
              </div>
            ...
            <div v-else>
              <div class="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-200 p-6 dark:border-gray-700">
                ...
                <p class="text-sm text-muted-foreground">No permits for this zone</p>
              </div>
            </div>
          </div>
        </div>
