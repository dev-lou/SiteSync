...
export function mountWidgetName(container: HTMLElement, props: { convexUrl: string; projectId: string; userId: string; userRole: string }): ComponentPublicInstance {
...
3. **Better Auth env vars** — `BETTER_AUTH_SECRET` and `CONVEX_URL` environment variables must be set for auth to work. Without these, login will fail and the auth guard in `hooks.server.ts` will redirect all requests to `/login`.
