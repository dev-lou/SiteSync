...
crons.interval('generate-daily-logs', { hours: 24 }, {
  handler: 'dailyLogs/scheduled:generateDailyLogs',
});
...
export const generateDailyLogs = internalMutation({
  handler: async (ctx) => {
    ...
    for (const project of projects) {
      ...
      const allDeliveries = await ctx.db.query('deliveries').withIndex('by_project', (q) => q.eq('projectId', project._id)).collect();
      const receivedToday = allDeliveries.filter(
        (d) => d.actualArrival && d.actualArrival >= startOfDay && d.actualArrival < endOfDay,
      );

      const allInspections = await ctx.db.query('inspections').withIndex('by_project', (q) => q.eq('projectId', project._id)).collect();
    ...
    return { projectsProcessed: projects.filter((p) => p.status === 'active').length };
  },
});
