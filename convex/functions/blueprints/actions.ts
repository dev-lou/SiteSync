...
export const convertPdfToPages = action({
  ...
  handler: async (ctx, args) => {
    const user = await requireRole(
      { auth: ctx.auth, db: ctx.db } as any,
      [ROLES.admin, ROLES.architect],
    );
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        const storageId = await ctx.storage.store(new File([pageBlob], `page-${i}.png`));
...
export const autoTagInspectionPhoto = action({
  ...
  handler: async (ctx, args) => {
    try {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                ...
                { inline_data: { mime_type: blob.type || 'image/jpeg', data: base64Image } },
              ],
            }],
          }),
        },
      ...
      return { tags: [], description: `Analysis failed: ${(err as Error).message}`, safetyConcerns: [] };
    }
  },
});
