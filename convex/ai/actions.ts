    ...
    if (geminiKey) {
      try {
        ...
        if (response.ok) {
          ...
          return {
            defects:
              text.includes('crack') || text.includes('damage') || text.includes('defect')
                ? [
                    {
                      type: text.includes('crack') ? 'crack' : 'defect',
                      severity: text.includes('critical')
                        ? 'critical'
            ...
            recommendations: text.includes('recommend')
              ? text
                  .split('\n')
                  .filter((l: string) => l.toLowerCase().includes('recommend'))
                  .map((l: string) => l.replace(/^[\d.-]*\s*/g, '').substring(0, 200))
...
export const generateTaskSuggestions = action({
  ...
  handler: async (ctx, args) => {
    ...
    const inspection = await (ctx as any).db.get(args.inspectionId);
    if (!inspection) throw new Error('Inspection not found');

    const failedItems = inspection.checklist.filter(
      (item: { passed?: boolean }) => item.passed === false,
    );
    const skippedItems = inspection.checklist.filter(
      (item: { passed?: boolean }) => item.passed === undefined,
    );
